...
movePic = function() {
    ...
    let topOffset = Math.min((shake / (canvas.height * 0.005) + (shake)) * verDir / IMG_BASE_DIVIDOR - TOP_OFFSET, 0)
    let leftOffset = Math.min((shake / (canvas.width * 0.005) + (shake)) * horDir / IMG_BASE_DIVIDOR - LEFT_OFFSET, 0)
    document.body.style.backgroundPosition = leftOffset + "px " + topOffset + "px"
}
